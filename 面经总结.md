...
[原文链接](https://www.cnblogs.com/woki/p/13880763.html)

为解决协程间的同步问题，提供了标准库代码，sync和sync/atomic

* 互斥锁：传统并发编程对共享资源进行访问控制的主要手段，sync.Mutex：Lock和Unlock
* 读写锁：使用场景一般为读多写少，可以让多个读操作并发同时读取，但是写操作是完全互斥的，读与写也是互斥的
* Go中的死锁：
  * 单gorutine同时读写，写死锁：应该至少在两个gorutinue进行channle通讯，或者使用缓冲区
  * 多gorutinue使用一个channle通信，写先于读
  * 多channel交叉死锁，多个goroutinue使用多个channel互相等待对方写入导致死锁
  * 隐形死锁
...
[原文链接](https://studygolang.com/articles/12785)

* ***panic***：程序发生异常时，无法继续运行，使用panic来终止程序。当函数发生 panic 时，它会终止运行，在执行完所有的延迟函数后，程序控制返回到该函数的调用方，直到当前协程的所有函数都返回退出，然后程序会打印出 panic 信息，接着打印出堆栈跟踪，最后程序终止
  * 发生了不可恢复的错误，程序不能继续运行
  * 发生了一个编程上的错误
* ***recover***：用于重新获得panic协程的控制，只能在defer内部调用，可以取到panic的错误信息，并停止panic续发时间，程序运行恢复正常
* panic，recover和Go协程只有在相同的协程中才管用，recover不能恢复一个不同协程的panic
...
[原文链接](https://studygolang.com/articles/14583)

* map的底层是用哈希表（散列表）实现的，主要结构体有两个，一个是hmap，一个是bucket
* hmap：![](https://static.studygolang.com/180826/09d0c94fc2946bba795ecc2ae0c97ac2.png)
* bucket：![](https://static.studygolang.com/180826/0b9839a2b09116a7f257dc46ae8fb23e.png)
* 高位哈希值数组记录的是当前bucket中key相关的“索引”；指向扩容的指针形成一个链表
*  golang把key值哈希后的值按照用途一分为二：高位和低位：低位用于寻找当前key属于哪一个bucket，高位用于寻找bucket中的哪个key
* 扩容：当哈希表增长的时候，go会将bucket数组的数量扩充一倍，产生新的bucket数组，并将旧数组的数据迁移至新数组
...
[原文链接](https://www.cnblogs.com/mafeng/p/9018110.html)

[原文链接](https://blog.csdn.net/try_mistake/article/details/83718158)

* map由多协程同时读写时会出现 fatal error:concurrent map read and map write的错误
* 多个协程同时写会出现fatal error: concurrent map writes的错误
* map为引用类型，即使函数传值调用，参数副本依然指向映射m，多个协程并发写同一个映射map，共享资源遭到破坏
* 解决方案：
  * 加锁
    * 通用锁
    * 读写锁
  * 利用channel串行化处理，用一个协程操作map，其他协程通过channle进行通信
  * 利用sync.map，协程安全的map
...
* 判断哈希算法好坏的四个定义：
  ...
  * 单调性：保证已分配的内容可以被映射到原有或者新的缓冲中去，而不会被映射到旧的缓冲计划中的其他缓冲区
...
* MyISAM：
  ...
  * 采用非聚集索引，索引文件的数据或存储指向数据文件的指针。辅索引与主索引基本一致，但是辅索引不用保证唯一性

  * 主索引：![](https://images2017.cnblogs.com/blog/1149689/201708/1149689-20170814155346193-1896148402.png)

  * 辅索引：![](https://images2017.cnblogs.com/blog/1149689/201708/1149689-20170814155416240-955064122.png)
...
* InnoDb：
  ...
  * 主索引：![](https://images2017.cnblogs.com/blog/1149689/201708/1149689-20170814155436850-1643254038.png)
  * 辅助索引：![](https://images2017.cnblogs.com/blog/1149689/201708/1149689-20170814155501553-943278018.png)





索引查找过程：![](https://images2017.cnblogs.com/blog/1149689/201708/1149689-20170814155543365-1816361585.png)
...
* 缓存穿透：key对应的数据不存在，每次针对此key的请求都从缓存中获取不到，请求都会到数据源，从而可能会压垮数据源。
  * 采用布隆过滤器，将所有可能存在的数据哈希到一个足够大的bitmap中，过滤掉不存在的数据
  * 如果一个查询返回的数据为空，仍然把空结果进行缓存，过期时间短

* 缓存击穿：key对应的数据存在，但在redis中过期，若有大量并发请求出现，都会从后端DB中加载数据并回设到缓存，把后端压垮

  * 使用互斥锁mutex，在缓存失效的时候（判断拿出来的值为空），不是立即去load db，而是先使用缓存工具的某些带成功操作返回值的操作（比如Redis的SETNX或者Memcache的ADD）去set一个mutex key，当操作返回成功时，再进行load db的操作并回设缓存；否则，就重试整个get缓存的方法

* 缓存雪崩：当缓存服务器重启或大量缓存集中在一个时间段失效，失效时会给后端系统带来很大压力
...
是tcp/ip协议中传输层的概念，用于区分同一台主机中的不同应用
...
死锁是指多个进程在运行过程中因争夺资源而造成的一种僵局，如果没有外力作用，都无法再向前推进
...
* 死锁产生的四个必要条件
  ...
  * 请求和保持条件：当进程因请求资源而阻塞是，对已获得的资源保持不放
  * 不剥夺条件：进程已获得的资源在未使用完之前不能被剥夺，只能够由自己释放
...
* 解决死锁的基本方法
  ...
  * 预防死锁
    ...
    * 资源一次性分配原则
    * 只要有一个资源得不到分配，就不给这个进程分配其他的资源
    * 可剥夺资源：某进程获得了部分资源，但是得不到其他的资源，那就释放已占有的资源
    * 资源有序分配发

    1. 以确定的顺序获得锁：如果需要获取多个锁，那么设计是要考虑不同线程间获得锁的顺序
    2. 超时放弃：按照固定时长等待锁，线程在获取锁超时之后主动释放之前获得的所有锁

  * 避免死锁
    * 银行家算法：如果某个进程请求系统资源改变了状态，确定结果是否还处于安全状态
  ...
  * 解除死锁
    ...
    * 剥夺资源：从其他进程剥夺足够多的资源给死锁进程，以解除死锁状态
...
[原文链接](https://zhuanlan.zhihu.com/p/76023663)
...
* Reno：适用于低延时，低带宽的网络，将拥塞控制分为四个阶段：慢启动、拥塞避免、快重传和快恢复  
  ...
  * ![](https://pic1.zhimg.com/80/v2-86c12ed3ddb2391c057f7d3598372880_720w.jpg)
* BBR：不将出现丢包或时延增加作为拥塞的信号，而是认为当网络上的数据包总量大于瓶颈链路带宽和时延的乘积时才出现了拥塞，所以 BBR 也称为基于拥塞的拥塞控制算法，其适用网络为高带宽、高时延、有一定丢包率的长肥网络，可以有效降低传输时延，并保证较高的吞吐量
  * 周期性地探测网络的容量，交替测量一段时间内的带宽极大值和时延极小值，将其乘积作为作为拥塞窗口大小，使得拥塞窗口始的值始终与网络的容量保持一致
  * ![](https://pic3.zhimg.com/80/v2-87e9aa069720faf6bf16b8d10a72265e_720w.jpg)
...
[原文链接](https://www.cnblogs.com/xflonga/p/9368993.html)

* 1XX：通知，响应代码尽在与HTTP服务器沟通时使用
* 2XX：成功，表示操作成功
* 3XX：重定向
* 4XX：客户端错误
* 5XX：服务端错误
...
* TCP 和 UDP 的区别
  ...
  * TCP是面向字节流的，UDP是就有数据报的
  * TCP保证数据正确性，UDP可能丢包
  * TCP保证数据顺序，UDP不能保证
* 什么是面向连接，什么是面向无连接
  * 在互通之前，面向连接的协议会先建立连接，如有TCP三次握手等，UDP不会
* TCP为什么是可靠连接
...
[原文链接](https://blog.csdn.net/qq_29677867/article/details/90112120)
...
* 深度或广度优先搜索算法：从起点开始访问所有深度或广度路径，到达终点的路径有多条，取最短的一条
* 佛洛依德算法：
